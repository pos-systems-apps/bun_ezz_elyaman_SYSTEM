...
import 'package:pos_system/features/return_resources_invoices/ui/widgets/return_resources_invoices_item_widget.dart';


class ReturnResourcesInvoicesBodyWidget extends StatelessWidget {
  const ReturnResourcesInvoicesBodyWidget({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<ReturnResourcesInvoicesCubit, ReturnResourcesInvoicesState>(
      ...
      builder: (context, state) {
        if (ReturnResourcesInvoicesCubit.get(context).reservations.isEmpty &&
        ...
        } else if (ReturnResourcesInvoicesCubit.get(context).reservations.isEmpty &&
            state is! OnGetInvoicesLoadingState) {
          return Center(
            child: Column(
          ...
          return Expanded(
            child: SingleChildScrollView(
              padding: EdgeInsets.symmetric(horizontal: 20.w),
              child: Column(
                children: ReturnResourcesInvoicesCubit.get(context)
                    .reservations
                    .map((item) => ReturnResourcesInvoicesItemWidget(item: item,))
                    .toList(),
              ),
            ),
          );

...
class MyRequestsService {
  ...
  Future<CategoryResponse> getCategories(int page) async {
    final response =
        await apiConsumer.get(MyRequestsApiEndPoints.getCategoriesUrl(page), {
  ...
  Future<CategoryProductsResponse> getCategoryProducts(
    ...
    final response = await apiConsumer
        .get(MyRequestsApiEndPoints.getCategoryProductsUrl(categoryId, type, page), {
      ConstantKeys.appAuthorization:
          "${ConstantKeys.appBearer} ${await CacheHelper.getSecuredString(ConstantKeys.saveTokenToShared)}",
    });
    if (response.statusCode == StatusCode.ok) {
  ...
  Future<SearchProductsResponse> getSearchProducts(
    ...
    final response = await apiConsumer
        .get(MyRequestsApiEndPoints.getSearchProductsUrl(name, type, page), {
      ConstantKeys.appAuthorization:
          "${ConstantKeys.appBearer} ${await CacheHelper.getSecuredString(ConstantKeys.saveTokenToShared)}",
    });
    if (response.statusCode == StatusCode.ok) {

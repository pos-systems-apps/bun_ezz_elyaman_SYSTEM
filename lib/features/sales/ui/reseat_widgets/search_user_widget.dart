            ...
            SearchWidget(
              ...
              onchange: (String? value) {
                ...
                _delay = Timer(Duration(seconds: 1), () {
                  SalesCubit.get(context).getUsers();
                });
              },
            ),
            if (SalesCubit.get(context).users.isNotEmpty)
              Container(
                height: 250.h,
                ...
                child: ListView.separated(
                  shrinkWrap: true,
                  itemCount: SalesCubit.get(context).users.length,
                    ...
                    return ListTile(
                      title: Text(
                        context.locale.languageCode == "ar"
                            ? SalesCubit.get(context).users[index].nameAr
                            : SalesCubit.get(context).users[index].nameEn,
                        maxLines: 2,
                        style: TextStyles.font16BlackWeight500,
                      ),
                      onTap: () {
                        SalesCubit.get(context).onSelectUser(
                            SalesCubit.get(context).users[index]);
                      },
                    );
                  },
                ),

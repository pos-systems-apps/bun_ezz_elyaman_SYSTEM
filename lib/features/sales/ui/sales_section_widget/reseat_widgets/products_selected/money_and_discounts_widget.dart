              ...
              return Column(
                ...
                children: [
                  ...
                  _moneyWidget("خصم اضافي",
                      " جنيه  ${(ReseatSelectedProducts(selectedProducts: SalesCubit.get(context).selectedProducts).getReseatData(discountTypeId: SalesCubit.get(context).selectedPercentType?.id, discount: SalesCubit.get(context).percentController.text)['extraDiscount'])!.toStringAsFixed(3)}"),
            ...
            listener: (_, state) {
              if (state is OnCreateOrderSuccessState) {
                context.pushNamed(Routes.electronicInvoiceScreen,
                    arguments: {"invoiceId": state.orderId,'type':4});

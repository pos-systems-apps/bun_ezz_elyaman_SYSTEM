...
import 'package:pos_system/features/leave_requests/data/services/leave_requests_api_end_points.dart';
...
class LeaveRequestsService {
  ...
  LeaveRequestsService({required this.apiConsumer});

  Future<CategoryResponse> getCategories(int page) async {
    final response =
        await apiConsumer.get(LeaveRequestsApiEndPoints.getCategoriesUrl(page), {
  ...
  Future<CategoryProductsResponse> getCategoryProducts(
    ...
    final response = await apiConsumer
        .get(LeaveRequestsApiEndPoints.getCategoryProductsUrl(categoryId, type, page), {
      ConstantKeys.appAuthorization:
          "${ConstantKeys.appBearer} ${await CacheHelper.getSecuredString(ConstantKeys.saveTokenToShared)}",
    });
    if (response.statusCode == StatusCode.ok) {
  ...
  Future<SearchProductsResponse> getSearchProducts(
    ...
    final response = await apiConsumer
        .get(LeaveRequestsApiEndPoints.getSearchProductsUrl(name, type, page), {
      ConstantKeys.appAuthorization:
          "${ConstantKeys.appBearer} ${await CacheHelper.getSecuredString(ConstantKeys.saveTokenToShared)}",
    });
    if (response.statusCode == StatusCode.ok) {
  ...
  Future<SuccessResponseModel> createOrder(CreateOrderRequest parameter) async {
    final response = await apiConsumer.multiPost(
        LeaveRequestsApiEndPoints.createOrderURl,
        CreateOrderRequest(

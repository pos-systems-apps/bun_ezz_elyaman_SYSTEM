...
class CreateVisitSearchUserWidget extends StatelessWidget {
  const CreateVisitSearchUserWidget({super.key});

  @override
  Widget build(BuildContext context) {
    Timer? _delay;
    return BlocBuilder<CreateVisitCubit, CreateVisitState>(
        ...
        return Column(
          children: [
            SearchWidget(
              ...
              controller: CreateVisitCubit.get(context).searchUserController,
              ...
              onchange: (String? value) {
                ...
                _delay = Timer(Duration(seconds: 1), () {
                  CreateVisitCubit.get(context).getUsers();
                });
              },
            ),
            if (CreateVisitCubit.get(context).users.isNotEmpty)
              Container(
                height: 250.h,
                ...
                child: ListView.separated(
                  shrinkWrap: true,
                  itemCount: CreateVisitCubit.get(context).users.length,
                    ...
                    return ListTile(
                      title: Text(
                        context.locale.languageCode == "ar"
                            ? CreateVisitCubit.get(context).users[index].nameAr
                            : CreateVisitCubit.get(context).users[index].nameEn,
                        maxLines: 2,
                        style: TextStyles.font16BlackWeight500,
                      ),
                      onTap: () {
                        CreateVisitCubit.get(context).onSelectUser(
                            CreateVisitCubit.get(context).users[index]);
                      },
                    );
                  },
                ),
